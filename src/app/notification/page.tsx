...
export default function NotificationPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (validateStep()) {
      setIsLoading(true);
      setTimeout(() => {
        console.log("Xabarnoma yuborildi: ", formData);
        setIsLoading(false);
        ...
        <div className="mt-8 flex items-center justify-between">
          ...
          {currentStep < 4 ? (
            ...
            <button 
              ...
              disabled={isLoading}
              className="px-8 py-3 rounded-lg font-medium flex items-center gap-2 bg-blue-600 text-white hover:bg-blue-700 transition-colors shadow-sm shadow-blue-600/20 disabled:opacity-70"
            >
              {isLoading ? 'Yuborilmoqda...' : <><Send className="w-4 h-4" /> Xabarnomani Yuborish</>}
            </button>
          )}
        </div>
      </main>
