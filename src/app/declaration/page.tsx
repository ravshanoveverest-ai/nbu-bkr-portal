...
//     relativeCompanies: [{ relativePinfl: '', companyName: '', stir: '' }],
//     conflictInfo: '',
//     additionalInfo: ''
//   });
...
//         if (comp.companyName && !pinflRegex.test(comp.relativePinfl)) newErrors.push(`Qarindoshga tegishli ${idx + 1}-kompaniyada JSHSHIR xato (14 ta raqam)`);
...
//     if (type === 'myCompanies') {
//       setFormData({ ...formData, myCompanies: [...formData.myCompanies, { companyName: '', stir: '' }] });
//     } else {
//       setFormData({ ...formData, relativeCompanies: [...formData.relativeCompanies, { relativePinfl: '', companyName: '', stir: '' }] });
//     }
//   };
...
//           {/* 3-QADAM: Yuridik shaxslar (MANUAL INPUT) */}
//           {currentStep === 3 && (
...
//                     <div key={idx} className={`flex flex-col md:flex-row gap-4 items-end bg-slate-50/50 p-4 rounded-xl border transition-all ${comp.stir ? 'border-emerald-200 bg-emerald-50/30' : 'border-slate-200'}`}>
...
//                     <div key={idx} className={`flex flex-col md:flex-row gap-4 items-end bg-slate-50/50 p-4 rounded-xl border transition-all ${comp.stir ? 'border-emerald-200 bg-emerald-50/30' : 'border-slate-200'}`}>
//                       <div className="w-full md:w-1/3">
//                         <label className="block text-xs font-medium text-slate-500 mb-1">Qarindoshning JSHSHIRi</label>
//                         <input 
//                           type="text" 
//                           maxLength={14}
//                           placeholder="14 ta raqam"
//                           value={comp.relativePinfl}
//                           onChange={(e) => {
//                             const newComps = [...formData.relativeCompanies];
//                             newComps[idx].relativePinfl = e.target.value.replace(/\D/g, '');
...
export default function DeclarationPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    myCompanies: [{ companyName: '', stir: '', isLeader: '', sharePercent: '', noDetails: false }],
    relativeCompanies: [{ relativeName: '', companyName: '', stir: '', isLeader: '', sharePercent: '', noDetails: false }],
    conflictInfo: '',
    additionalInfo: ''
  });
  ...
  const addCompany = (type: 'myCompanies' | 'relativeCompanies') => {
    if (type === 'myCompanies') {
      setFormData({ ...formData, myCompanies: [...formData.myCompanies, { companyName: '', stir: '', isLeader: '', sharePercent: '', noDetails: false }] });
    } else {
      setFormData({ ...formData, relativeCompanies: [...formData.relativeCompanies, { relativeName: '', companyName: '', stir: '', isLeader: '', sharePercent: '', noDetails: false }] });
    }
  };
